document.addEventListener("DOMContentLoaded", async function () {
  ...
  let currentService = localStorage.getItem(DEFAULT_MUSIC_PROVIDER_KEY) || "tidal";
  ...
  let playlistSearchAbortController = null;
  ...
  const Playlists = {
    ...
    get: function (id) {
      return this.data.find(function (p) {
        return p.id === id;
      });
    },
    ...
    delete: function (id) {
      ...
      this.data = this.data.filter(function (p) {
        return p.id !== id;
      });
      this.save();
    },
    ...
    addTrack: function (id, track) {
      const p = this.get(id);
      if (p) {
        if (!p.tracks.some(function (t) { return t.id === track.id; })) {
          p.tracks.push(track);
          this.save();
        }
      }
    ...
    removeTrackById: function (id, trackId) {
      const p = this.get(id);
      if (p) {
    ...
    reorderTracks: function (id, oldIndex, newIndex) {
      const p = this.get(id);
      if (p) {
    ...
    isFavorite: function (trackId) {
      ...
      return p ? p.tracks.some(function (t) { return t.id === trackId; }) : false;
    },
  };
  ...
  openPlaylistsBtn.addEventListener("click", function () {
    renderPlaylistList();
  ...
  backToSearchFromQueueBtn.addEventListener("click", function () {
  ...
  function renderPlaylistList() {
    ...
    Playlists.data.forEach(function (playlist) {
      ...
      if (!isFav) {
        el.querySelector(".remove-track-btn").addEventListener("click", function (e) {
          e.stopPropagation();
  ...
  function renderPlaylistDetailView(id, isEditMode) {
    ...
    toggleEditModeBtn.onclick = function () {
  ...
  backToPlaylistsBtn.addEventListener("click", function () {
    renderPlaylistList();
  ...
  function renderQueue() {
    ...
    if (upcomingTracks.length === 0) {
      ...
      upcomingTracks.forEach(function (track, i) {
    ...
    list._sortable = Sortable.create(list, {
      ...
      onEnd: function (evt) {
        ...
        const itemsList = Array.from(list.children).filter(function (el) {
          return el.classList.contains("result-item") && !el.classList.contains("playing-now");
        });
        const newUpcomingQueue = itemsList.map(function (el) {
          return trackQueue[parseInt(el.dataset.index)];
        });
  ...
  lockQueueBtn.addEventListener("click", function () {
  ...
  addSongToPlaylistBtn.addEventListener("click", function () {
  ...
  finishAddingSongsBtn.addEventListener("click", function () {
  ...
  playlistSearchInput.addEventListener("input", function () {
    ...
    playlistSearchTimeout = setTimeout(function () {
  ...
  playlistServiceBtn.addEventListener("click", function (e) {
    e.stopPropagation();
  ...
  const originalDropdownHandlers = serviceDropdown.querySelectorAll(".dropdown-option");
  originalDropdownHandlers.forEach(function (opt) {
  ...
  function updateVolumeBar(value) {
    ...
    const accentColor = getComputedStyle(document.documentElement).getPropertyValue("--accent").trim() || "#6AA5C9";
    volumeSlider.style.background = `linear-gradient(to right, ${accentColor} ${percentage}%, rgba(255, 255, 255, 0.2) ${percentage}%)`;
  }
  ...
  function drawVisualizer() {
    if (!audio.paused && audioContext && analyser) {
      ...
      const bands = [2, 6, 14, 30, 60];
      const powers = [1.8, 1.6, 1.4, 1.4, 1.4];
      const mults = [1.0, 1.1, 1.2, 1.4, 1.6];
  ...
  function togglePlayback() {
    if (!audio.src && trackQueue.length > 0) {
      loadAndPlayTrack(0);
    } else if (audio.src) {
      if (audio.paused) {
        audio.play();
  ...
  function updateNowPlayingHeart(track) {
    ...
    nowPlayingHeart.onclick = function () {
  ...
  function loadAndPlayTrack(index) {
    ...
    const hasLossless = track.qualities && track.qualities.some(function (q) {
      return q === "LOSSLESS" || q === "HIRES_LOSSLESS" || q === "HI_RES_LOSSLESS";
    });

    if (hasLossless) {
      hdTag.classList.add("visible");
    } else {
      hdTag.classList.remove("visible");
    }
  ...
  async function fetchTidalStreamUrl(track) {
    ...
    if (track.qualities && track.qualities.includes("HIRES_LOSSLESS")) {
        quality = "HI_RES_LOSSLESS";
    } else if (track.qualities && track.qualities.includes("LOSSLESS")) {
        quality = "LOSSLESS";
    ...
    try {
      const res = await fetch(`/worker/audiom/track/${track.id}/stream?quality=${quality}`);
      
      if (!res.ok) {
          throw new Error(`Failed to fetch tidal stream: ${res.status}`);
      }

      songTitle.textContent = track.title;
      songArtist.textContent = track.artist;
      
      audio.src = `/worker/audiom/track/${track.id}/stream?quality=${quality}`;
    ...
    try {
      ...
      if (data && data.streamUrl) {
        ...
        audio
          .play()
          .then(function () {
            if (data.warning && data.warning.includes("preview")) {
              showModal(
  ...
  async function fetchLyrics(track) {
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        bestMatch = data.find(function (item) {
          return item.syncedLyrics && item.trackName.toLowerCase() === track.title.toLowerCase();
        });
        if (!bestMatch) {
          bestMatch = data[0];
        }
  ...
  function renderLyrics() {
    lyricsContent.innerHTML = "";
    currentLyrics.forEach(function (line, index) {
  ...
  function syncLyrics(time) {
    if (!lyricsAvailable || currentLyrics.length === 0) {
      return;
    }
    let newIndex = -1;
  ...
  function updateLyricsUI() {
    ...
    lines.forEach(function (line, index) {
      ...
      if (index < activeLyricIndex) {
        ...
        if (dist <= 5) {
          line.classList.add(`future-${dist}`);
        }
      }
    });
  }
  ...
  serviceButtonIcon.addEventListener("click", function (e) {
    e.stopPropagation();
    hideAllDropdowns();
  ...
  serviceDropdown.querySelectorAll(".dropdown-option").forEach(function (option) {
  ...
  async function performSearch(
    ...
    if (isPlaylistSearch) {
      if (playlistSearchAbortController) {
        playlistSearchAbortController.abort();
      }
      playlistSearchAbortController = new AbortController();
    } else {
    ...
    const signal = isPlaylistSearch ? playlistSearchAbortController.signal : currentSearchAbortController.signal;

    const encodedQuery = encodeURIComponent(query).replace(/%20/g, "+");
    let url =
      serviceToUse === "tidal"
        ? `/worker/audiom/search/tracks?q=${encodedQuery}&max=${RESULTS_PER_PAGE}&offset=${offset}`
    ...
    try {
      ...
      if (serviceToUse === "tidal") {
        results = data.data.items.map(function (item) {
          const rawArtwork = item.album && item.album.cover
        ...
        if (!isPlaylistSearch) {
          hasMore = offset + results.length < data.data.totalNumberOfItems;
        }
      } else {
      ...
      if (!isPlaylistSearch) {
        ...
        playlistLoadingMore.style.display = "none";
      }
    }
  }
  ...
  lockPlaylistBtn.addEventListener("click", function () {
